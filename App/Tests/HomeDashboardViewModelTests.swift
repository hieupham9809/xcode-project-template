...
final class HomeDashboardViewModelTests: XCTestCase {
    ...
    func testTotalSpend_Calculation() async {
        ...
        // 10 + (120/12) = 20
        let expectedMonthly = formatter.string(from: 20.0 as NSDecimalNumber) ?? "$20.00"
        XCTAssertEqual(viewModel.totalSpend, expectedMonthly)
        
        // When (Period is Yearly)
        viewModel.selectedPeriod = .yearly
        // (10*12) + 120 = 240
        let expectedYearly = formatter.string(from: 240.0 as NSDecimalNumber) ?? "$240.00"
        XCTAssertEqual(viewModel.totalSpend, expectedYearly)
    }
