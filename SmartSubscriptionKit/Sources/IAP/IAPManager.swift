//
//  SmartSubscriptionIAPManager.swift
//
//
...
public protocol SmartSubscriptionIAPManagerDelegate: AnyObject {
...
public class SmartSubscriptionIAPManager: NSObject {
    public static let shared = SmartSubscriptionIAPManager()
    public weak var delegate: SmartSubscriptionIAPManagerDelegate?
    var productItems: [SmartSubscriptionIAPItem] = []
    ...
    public func fetchProducts() {
        ...
        let productIdentifiers = Set<String>(
            [SmartSubscriptionIAPItem.Identifier.aCupOfCoffee, .aBeer]
            .map { $0.rawValue }
        )
...
extension SmartSubscriptionIAPManager: SKProductsRequestDelegate {
    public func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        let productItems = response.products.map { product -> SmartSubscriptionIAPItem in
            return SmartSubscriptionIAPItem(product: product)
        }
...
extension SmartSubscriptionIAPManager: SKPaymentTransactionObserver {
    public func purchaseProduct(_ product: SmartSubscriptionIAPItem) {
